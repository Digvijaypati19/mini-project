...
{
    $time=time();
    $actual_image_name=$_FILES['s_img1']['name'];
    $reimg=$time.$actual_image_name;
    $size=$_FILES['s_img1']['size'];
    $tmp=$_FILES['s_img1']['tmp_name'];
    ...
    if(in_array($ext[1],$valid_formats)){
        ...
        if(move_uploaded_file($tmp,$path.$reimg))
        {
         ...
         $qry=mysqli_query($con,"INSERT INTO patil_highscool.staff(s_name,s_pass,s_img1,s_flag) VALUES ('$myusername','$password','$reimg','1')");
